...
public class Main {
	...
	final static Point pointOfOffice =  new Point(officeLatitude, officeLongitude);
	
	final static double distanceLimitInKilometers = 100;
	...
	public static void main(String[] args) {		
		...
		for (Customer customer: customers){			
			Point pointOfCustomer = new Point(customer.getLatitude(), customer.getLongitude());
			// Calculating the distance between customers location and Dublin office
			double distance = distanceCalculator.calculateDistance(pointOfOffice, pointOfCustomer);
			
			// Checking if the customer is within the predefined limit, which is 100 KMs here
			if (distance < distanceLimitInKilometers){
